...
import { OUTPUT_SURGE_DIR, SOURCE_DIR, PUBLIC_DIR } from './constants/dir';
...
export const buildSpeedtestDomainSet = task(require.main === module, __filename)(
  async (span) => {
    await new DomainsetOutput(span, 'speedtest')
    ...
    const outPath = path.resolve(PUBLIC_DIR, 'List/domainset/speedtest.conf');
    console.log('✅ speedtest.conf 生成检查:', fs.existsSync(outPath), outPath);
  }
);
